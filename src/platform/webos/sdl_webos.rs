...
//!
//! Same rule as `ndl::ffi` and `smp::ffi` (`docs/NOTES.md`): only the bundled fork exports
//! these, so linking them would stop the process before `main()` under a stock SDL2. Resolved
//! here, a miss is a runtime error every caller degrades on.
use std::ffi::{c_char, c_int, CStr};
use std::sync::OnceLock;

use anyhow::Result;
use sdl2::sys::{SDL_Rect, SDL_bool};
...
pub struct Fns {
    ...
    pub destroy_exported_window: unsafe extern "C" fn(*const c_char),
}
...
pub fn fns() -> Result<&'static Fns> {
    ...
    dl::cached(&FNS, LIB_NAME, |lib| {
        Ok(Fns {
            ...
            create_exported_window: lib.sym(c"SDL_webOSCreateExportedWindow")?,
            set_exported_window: lib.sym(c"SDL_webOSSetExportedWindow")?,
            destroy_exported_window: lib.sym(c"SDL_webOSDestroyExportedWindow")?,
        })
    })
}
