...
use crate::services::store::VideoBackend;
...
impl MediaPipeline {
    ...
    pub fn build(
        ...
        let route = wanted.on_plane(plane.is_some());
        tracing::info!(
            "audio path: {} (host resolved {} channel(s))",
            audio_path_label(player.name(), route, plane.is_some()),
